...
class SaveCommentByPracticeTestQuestionIdRequest extends FormRequest
{
    /**
    ...
    protected function getValidatorInstance()
    {
        ...
        try{
            ...
            if (($questionId = $cryptoService->decryptValue($this->question_id)) && ($userId = $cryptoService->decryptValue($this->user_id))) {
                ...
                Log::channel('test')->error('[GetCommentsByQuestionIdRequest:getValidatorInstance] Get comments by question id request validation failed because an exception occured. Visit normal log file to check the exception.');
            }
        } catch (Exception $e) {
            $this->question_id = null;
            $this->user_id = null;

            Log::channel('test')->error('[GetCommentsByQuestionIdRequest:getValidatorInstance] Get comments by question id request validation failed because an exception occured: ');
